...
globalThis.ccbSetSceneNodeProperty = (node, propName, arg0, arg1, arg2) => {
	...
	} else if (propName == "UpVector") {
		if (cam != null) {
			cam.UpVector = new CL3D.Vect3d(x, y, z);
		}
	} else if (propName == "FieldOfView_Degrees") {
		if (cam != null) {
			cam.setFov(CL3D.degToRad(firstParam));
		}
	} else if (propName == "AspectRatio") {
		if (cam != null) {
	...
	} else if (propName == "Looping") {
		if (animnode != null) {
			animnode.setLoopMode(firstParam);
		}
	} else if (propName == "FramesPerSecond") {
		if (animnode != null) {
			animnode.setAnimationSpeed(firstParam * 0.001);
		}
	} else if (propName == "AnimationBlending") {
		if (animnode != null) {
			animnode.AnimationBlendingEnabled = firstParam;
		}
	} else if (propName == "BlendTimeMs") {
		if (animnode != null) {
 ...
 */
globalThis.ccbGetCollisionPointOfWorldWithLine = (startX, startY, startZ, endX, endY, endZ) => {
 ...
 */
globalThis.ccbDoesLineCollideWithBoundingBoxOfSceneNode = (node, startX, startY, startZ, endX, endY, endZ) => {
	if (node == null) {
		return false;
	}
